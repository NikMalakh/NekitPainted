...
def type(_, msg):
	...
	while(tbp != orig_text):
		try:
			...
			sleep(e.x) 
@app.on_message(filters.command("antifem", prefixes = "/")&filters.me)
def ugnet(_, msg):
...
def raid(_, msg):
	...
	msg.delete()
	for i in range(1,50):
		app.send_message(chid,text)
		sleep(0.03)
@app.on_message(filters.command("spamphoto", prefixes = "/")&filters.me)
def photo(_, msg):
	...
	msg.delete()
	for i in range(1,25):
		app.send_photo(chid,text)
		sleep(0.05)
@app.on_message(filters.command("spamvideo", prefixes = "/")&filters.me)
def vid(_, msg):
	...
	msg.delete()
	for i in range(1,25):
		app.send_video(chid,text)
		sleep(0.075)
@app.on_message(filters.command("spamgif", prefixes = "/")&filters.me)
def gif(_, msg):
	...
	msg.delete()
	for i in range(1,25):
		app.send_animation(chid,text)
		sleep(0.05)
@app.on_message(filters.command("spamvoice", prefixes = "/")&filters.me)
def aud(_, msg):
	...
	msg.delete()
	for i in range(1,25):
		app.send_audio(chid,text)
		sleep(0.05) 
@app.on_message(filters.command("spamdoc", prefixes = "/")&filters.me)
def doc(_, msg):
...
def status(_, msg):
	...
	msg.delete()
@app.on_message(filters.command("last", prefixes = "/")&filters.me)
def search(_, msg):
...
def seek(_, msg):
	...
	for message in app.search_global(text , limit=25):
		app.send_message(chid, message.text)
		sleep(0.1)
@app.on_message(filters.command("countmsg", prefixes = "/")&filters.me)
def count(_, msg):
	chid=msg.chat.id
